function calculate(expr) {
    while(expr.includes("(")){
        openPar = expr.indexOf("(")
        closePar = expr.indexOf(")")
        par = expr.substring(openPar+1, closePar)
        
        //console.log("par "+par)
        resultPar = calculateSum(par)
        //console.log("resultPar" +resultPar)
        expr = expr.replace('('+par+")", resultPar)
        //console.log(expr)
    }  
    

    return calculateSum(expr)
}
